...
struct gpio_desc *gpio_to_desc(unsigned gpio)
{
	...
	// GPIO test for 54
	// GPIO1 controller 0 chip base 0	no
	// GPIO1 controller 1 chip base 32	54-32 = 22
	// GPIO2 controller 2 chip base 64
	// GPIO3 controller 3 chip base 96

	list_for_each_entry(chip, &gpio_chips, list) {
		printk("TIDU: gpio_to_desc %d %d  \n",chip->base,chip->ngpio);
		if (chip->base <= gpio && chip->base + chip->ngpio > gpio) {
			spin_unlock_irqrestore(&gpio_lock, flags);
			printk("TIDU: return_value of GPIO %d \n",(gpio - chip->base));
...
static void _gpio_set_open_drain_value(struct gpio_desc *desc, bool value)
{
	...
	printk("TIDU: _gpio_set_open_drain_value \r\n");
	if (value) {
...
static void _gpio_set_open_source_value(struct gpio_desc *desc, bool value)
{
	...
	printk("TIDU: _gpio_set_open_source_value \n");
	if (value) {
